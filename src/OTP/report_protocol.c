...
u8 Stick20HIDSendAccessStatusData(u8 * output) {
	...
	HID_Stick20SendData_st.SendDataType_u8
			= OUTPUT_CMD_STICK20_SEND_DATA_TYPE_STATUS;
	...
	CI_StringOut ((char *) text_au8);CI_StringOut (".");
	...
	{
		if (0 != (Config_st.VolumeActiceFlag_u8 & (1
	...
	CI_StringOut ((char *) text_au8);CI_StringOut (" - Admin ");
	...
	// Set endian
	Config_st.ActiveSmartCardID_u32 = change_endian_u32(
			Config_st.ActiveSmartCardID_u32);
	Config_st.ActiveSD_CardID_u32 = change_endian_u32(
			Config_st.ActiveSD_CardID_u32);

	memcpy(&HID_Stick20SendData_st.SendData_u8[0], &Config_st,
	...
	memcpy(&output[OUTPUT_CMD_RESULT_STICK20_DATA_START],
			(void *) &HID_Stick20SendData_st, sizeof(HID_Stick20SendData_est));
...
u8 Stick20HIDSendProductionInfos(u8 * output) {
	...
	HID_Stick20SendData_st.SendDataType_u8
			= OUTPUT_CMD_STICK20_SEND_DATA_TYPE_PROD_INFO;
	...
	memcpy(&HID_Stick20SendData_st.SendData_u8[0], &Infos_st,
...
u8 Stick20HIDSendMatrixData(u8 * output) {
	...
	HID_Stick20SendData_st.SendDataType_u8
			= OUTPUT_CMD_STICK20_SEND_DATA_TYPE_PW_DATA;
	...
	case 2:
		...
		memcpy(&HID_Stick20SendData_st.SendData_u8[1],
				&Stick20HIDSendMatrixDataBlock_au8[0], 20);
		HID_Stick20SendData_st.FollowBytesFlag_u8 = 1; // Yes
	...
	case 3:
		...
		memcpy(&HID_Stick20SendData_st.SendData_u8[1],
				&Stick20HIDSendMatrixDataBlock_au8[20], 20);
		HID_Stick20SendData_st.FollowBytesFlag_u8 = 1; // Yes
		HID_Stick20SendData_st.SendSize_u8 = 21;
	...
	case 4:
		...
		memcpy(&HID_Stick20SendData_st.SendData_u8[1],
				&Stick20HIDSendMatrixDataBlock_au8[40], 20);
		HID_Stick20SendData_st.FollowBytesFlag_u8 = 1; // Yes
		HID_Stick20SendData_st.SendSize_u8 = 21;
	...
	case 5:
		...
		memcpy(&HID_Stick20SendData_st.SendData_u8[1],
				&Stick20HIDSendMatrixDataBlock_au8[60], 20);
		HID_Stick20SendData_st.FollowBytesFlag_u8 = 1; // Yes
		HID_Stick20SendData_st.SendSize_u8 = 21;
	...
	case 6:
		...
		memcpy(&HID_Stick20SendData_st.SendData_u8[1],
				&Stick20HIDSendMatrixDataBlock_au8[80], 20);
		HID_Stick20SendData_st.FollowBytesFlag_u8 = 1; // Yes
		HID_Stick20SendData_st.SendSize_u8 = 1;
	...
	case 7:
		...
		calculated_crc32 = generateCRC_len(Stick20HIDSendMatrixDataBlock_au8,
				100 / 4);
...
void parse_report(u8 * report, u8 * output) {
	...
	if (calculated_crc32 == received_crc32) {
		...
		case CMD_WRITE_TO_SLOT: {
			...
			CI_StringOut ((char *) text);CI_StringOut ("\r\n");
		}
		...
		case CMD_GET_CODE:
			...
			if (((1 == authorized_user_crc_set) && (calculated_crc32
					== authorized_user_crc)) || (*((u8 *) (SLOTS_ADDRESS
					+ GLOBAL_CONFIG_OFFSET + 3)) != 1)) {
		...
		case CMD_SD_CARD_HIGH_WATERMARK:
			...
			cmd_getSdCardHighWaterMark(report, output);
			{
				...
				CI_StringOut ((char *) text);CI_StringOut (" % - read ");
		...
		default:
			if ((STICK20_CMD_START_VALUE > cmd_type) || (STICK20_CMD_END_VALUE
					< cmd_type)) {
				u8 text[10];
	...
	if ((STICK20_CMD_START_VALUE <= cmd_type) && (STICK20_CMD_END_VALUE
		...
		if ((calculated_crc32 == received_crc32)) {
			...
			case STICK20_CMD_ENABLE_ADMIN_READONLY_UNCRYPTED_LUN:
				...
				StartStick20Command(
						STICK20_CMD_ENABLE_ADMIN_READONLY_UNCRYPTED_LUN);
			...
			case STICK20_CMD_ENABLE_ADMIN_READWRITE_UNCRYPTED_LUN:
				...
				StartStick20Command(
						STICK20_CMD_ENABLE_ADMIN_READWRITE_UNCRYPTED_LUN);
			...
			case STICK20_CMD_ENABLE_ADMIN_READONLY_ENCRYPTED_LUN:
				...
				StartStick20Command(
						STICK20_CMD_ENABLE_ADMIN_READONLY_ENCRYPTED_LUN);
			...
			case STICK20_CMD_ENABLE_ADMIN_READWRITE_ENCRYPTED_LUN:
				...
				StartStick20Command(
						STICK20_CMD_ENABLE_ADMIN_READWRITE_ENCRYPTED_LUN);
		...
		memcpy(&output[OUTPUT_CMD_RESULT_STICK20_STATUS_START],
				(void *) &HID_Stick20Status_st, sizeof(HID_Stick20Status_est));
...
void cmd_get_status(u8 * report, u8 * output) {
	...
	memcpy(output + OUTPUT_CMD_RESULT_OFFSET + 6, (u8 *) SLOTS_ADDRESS
			+ GLOBAL_CONFIG_OFFSET, 3);
	memcpy(output + OUTPUT_CMD_RESULT_OFFSET + 9, (u8 *) SLOTS_ADDRESS
			+ GLOBAL_CONFIG_OFFSET + 3, 2);
...
u8 cmd_write_to_slot(u8 * report, u8 * output) {
	...
	u8 slot_tmp[SLOT_SIZE]; // this is will be the new slot contents
	u8 slot_name[15];
	u32 Counter_u32;
	u64 counter;

	memset(slot_tmp, 0, SLOT_SIZE);
	...
	if ((slot_no >= 0x10) && (slot_no < 0x10 + NUMBER_OF_HOTP_SLOTS)) // HOTP slot
	{
		...
		counter = Counter_u32;
		{
			...
			itoa(slot_no, text);
			CI_StringOut ((char *) text);CI_StringOut (" counter -");
			itoa(counter, text);
			CI_StringOut ((char *) text);CI_StringOut (" -");
		...
		write_to_slot(slot_tmp, hotp_slot_offsets[slot_no], SLOT_SIZE);
	} else if ((slot_no >= 0x20) && (slot_no < 0x20 + NUMBER_OF_TOTP_SLOTS)) // TOTP slot
	{
		slot_no = slot_no & 0x0F;

		write_to_slot(slot_tmp, totp_slot_offsets[slot_no], SLOT_SIZE);
	} else {
...
u8 cmd_read_slot_name(u8 * report, u8 * output) {
	...
	if ((slot_no >= 0x10) && (slot_no < 0x10 + NUMBER_OF_HOTP_SLOTS)) // HOTP slot
	{
		...
		if (is_programmed == 0x01) {
			memcpy(output + OUTPUT_CMD_RESULT_OFFSET,
					(u8 *) (hotp_slots[slot_no] + SLOT_NAME_OFFSET), 15);

			CI_StringOut ("HOTP Slot ");
			itoa(slot_no, text);
			CI_StringOut ((char *) text);CI_StringOut (" name -");CI_StringOut ((char *) output + OUTPUT_CMD_RESULT_OFFSET);CI_StringOut ("-\r\n");
		} else {
	...
	} else if ((slot_no >= 0x20) && (slot_no < 0x20 + NUMBER_OF_TOTP_SLOTS)) // TOTP slot
	{
		...
		if (is_programmed == 0x01) {
			memcpy(output + OUTPUT_CMD_RESULT_OFFSET,
					(u8 *) (totp_slots[slot_no] + SLOT_NAME_OFFSET), 15);
...
u8 cmd_read_slot(u8 * report, u8 * output) {
	...
	if ((slot_no >= 0x10) && (slot_no < 0x10 + NUMBER_OF_HOTP_SLOTS)) // HOTP slot
	{
		...
		if (is_programmed == 0x01) {
			memcpy(output + OUTPUT_CMD_RESULT_OFFSET,
					(u8 *) (hotp_slots[slot_no] + SLOT_NAME_OFFSET), 15);
			memcpy(output + OUTPUT_CMD_RESULT_OFFSET + 15,
					(u8 *) (hotp_slots[slot_no] + CONFIG_OFFSET), 1);
			memcpy(output + OUTPUT_CMD_RESULT_OFFSET + 16,
					(u8 *) (hotp_slots[slot_no] + TOKEN_ID_OFFSET), 13);
	...
	} else if ((slot_no >= 0x20) && (slot_no < 0x20 + NUMBER_OF_TOTP_SLOTS)) // TOTP slot
	{
		...
		if (is_programmed == 0x01) {
			memcpy(output + OUTPUT_CMD_RESULT_OFFSET,
					(u8 *) (totp_slots[slot_no] + SLOT_NAME_OFFSET), 15);
			memcpy(output + OUTPUT_CMD_RESULT_OFFSET + 15,
					(u8 *) (totp_slots[slot_no] + CONFIG_OFFSET), 1);
			memcpy(output + OUTPUT_CMD_RESULT_OFFSET + 16,
					(u8 *) (totp_slots[slot_no] + TOKEN_ID_OFFSET), 13);
			memcpy(output + OUTPUT_CMD_RESULT_OFFSET + 29,
					(u8 *) (totp_slots[slot_no] + INTERVAL_OFFSET), 2);
		} else {
			output[OUTPUT_CMD_STATUS_OFFSET] = CMD_STATUS_SLOT_NOT_PROGRAMMED;
			return 1;
		}
...
u8 CheckSystemtime(u32 Newtimestamp_u32) {
	...
	if ((Timediff_s32 > MAX_SYSTEMTIME_TIMEDIFF) || (Timediff_s32
			< -MAX_SYSTEMTIME_TIMEDIFF)) {
...
u8 cmd_get_code(u8 * report, u8 * output) {
	...
	if ((slot_no >= 0x10) && (slot_no < 0x10 + NUMBER_OF_HOTP_SLOTS)) // HOTP slot
	{
		...
		if (is_programmed == 0x01) {
			...
			memcpy(output + OUTPUT_CMD_RESULT_OFFSET + 4,
					(u8 *) hotp_slots[slot_no] + CONFIG_OFFSET, 14);
		} else {
			output[OUTPUT_CMD_STATUS_OFFSET] = CMD_STATUS_SLOT_NOT_PROGRAMMED;
		}
	} else if ((slot_no >= 0x20) && (slot_no < 0x20 + NUMBER_OF_TOTP_SLOTS)) // TOTP slot
	{
		 ...
		 */
		if (is_programmed == 0x01) {
			result = get_code_from_totp_slot(slot_no, timestamp
					/ (u64) interval);
			...
			memcpy(output + OUTPUT_CMD_RESULT_OFFSET, &result, 4);
			memcpy(output + OUTPUT_CMD_RESULT_OFFSET + 4,
					(u8 *) totp_slots[slot_no] + CONFIG_OFFSET, 14);
		} else {
			output[OUTPUT_CMD_STATUS_OFFSET] = CMD_STATUS_SLOT_NOT_PROGRAMMED;
		}
	} else {
...
u8 cmd_unlock_userpassword(u8 * report, u8 * output) {
	...
	Ret_u32 = LA_OpenPGP_V20_Test_ResetRetryCounter(
			(unsigned char *) newUserPassword_au8);
	if (TRUE == Ret_u32) {
		res = 0;
	}
...
u8 cmd_set_time(u8 * report, u8 * output) {
	...
	if ((old_time <= new_time_minutes) || (old_time == 0xffffffff)
			|| (report[CMD_DATA_OFFSET] == 1)) {
		/*
		 { u8 text[30];
...
u8 cmd_getPasswordSafeSlotLoginName(u8 * report, u8 * output) {
	...
	Ret_u32
			= PWS_GetSlotLoginName(report[1], &output[OUTPUT_CMD_RESULT_OFFSET]);
	if (TRUE == Ret_u32) {
		output[OUTPUT_CMD_STATUS_OFFSET] = CMD_STATUS_OK;
	} else {
		output[OUTPUT_CMD_STATUS_OFFSET] = CMD_STATUS_NOT_AUTHORIZED;
...
u8 cmd_getPasswordSafeSetSlotData_1(u8 * report, u8 * output) {
	...
	Ret_u32 = PWS_WriteSlotData_1(report[1], &report[2], &report[2
			+ PWS_SLOTNAME_LENGTH]);
...
u8 cmd_getSdCardHighWaterMark(u8 * report, u8 * output) {
	...
	output[OUTPUT_CMD_RESULT_OFFSET + DEVICE_HIGH_WATERMARK_SD_CARD_WRITE_MIN]
			= (u8) ((u64) sd_MaxAccessedBlockWriteMin_u32 * (u64) 100
					/ (u64) Blockcount_u32) + 1; // In
	// %
	// of
	// sd
	// size
	output[OUTPUT_CMD_RESULT_OFFSET + DEVICE_HIGH_WATERMARK_SD_CARD_WRITE_MAX]
			= (u8) ((u64) sd_MaxAccessedBlockWriteMax_u32 * (u64) 100
					/ (u64) Blockcount_u32) - 1; // In
	// %
	// of
	// sd
	// size
	output[OUTPUT_CMD_RESULT_OFFSET + DEVICE_HIGH_WATERMARK_SD_CARD_READ_MIN]
			= (u8) ((u64) sd_MaxAccessedBlockReadMin_u32 * (u64) 100
					/ (u64) Blockcount_u32) + 1; // In
	// %
	// of
	// sd
	// size
	output[OUTPUT_CMD_RESULT_OFFSET + DEVICE_HIGH_WATERMARK_SD_CARD_READ_MAX]
			= (u8) ((u64) sd_MaxAccessedBlockReadMax_u32 * (u64) 100
					/ (u64) Blockcount_u32) - 1; // In
	// %
	// of
	// sd
...
void CopyUpdateToHIDData(void) {
	memcpy(&HID_GetReport_Value_tmp[OUTPUT_CMD_RESULT_STICK20_STATUS_START],
			(void *) &HID_Stick20Status_st, sizeof(HID_Stick20Status_est));
}
...
void UpdateStick20Command(u8 Status_u8, u8 ProgressBarValue_u8) {
	...
	HID_GetReport_Value_tmp[OUTPUT_CRC_OFFSET + 1] = (calculated_crc32 >> 8)
			& 0xFF;
	HID_GetReport_Value_tmp[OUTPUT_CRC_OFFSET + 2] = (calculated_crc32 >> 16)
			& 0xFF;
	HID_GetReport_Value_tmp[OUTPUT_CRC_OFFSET + 3] = (calculated_crc32 >> 24)
			& 0xFF;
}
